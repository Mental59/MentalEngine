...
int GlCameraApp::run() {
  ...
  glFramework::GLShaderProgram shdModelVertex(
      "data/shaders/chapter04/GL01_duck.vert");
  glFramework::GLShaderProgram shdModelFragment(
      "data/shaders/chapter04/GL01_duck.frag");
  constexpr unsigned int numModelPrograms = 2;
  glFramework::GLShaderProgram modelPrograms[numModelPrograms]{
      shdModelVertex, shdModelFragment};
  glFramework::GLProgramPipeline progModel(modelPrograms, numModelPrograms);

  glFramework::GLShaderProgram shdCubeVertex(
      "data/shaders/chapter04/GL01_cube.vert");
  glFramework::GLShaderProgram shdCubeFragment(
      "data/shaders/chapter04/GL01_cube.frag");
  constexpr unsigned int numCubePrograms = 2;
  glFramework::GLShaderProgram cubePrograms[numCubePrograms]{shdCubeVertex,
                                                             shdCubeFragment};
  glFramework::GLProgramPipeline progCube(cubePrograms, numCubePrograms);
  ...
  glBindBufferRange(GL_UNIFORM_BUFFER, 0, perFrameDataBuffer, 0,
                    kUniformBufferSize);
  ...
  for (unsigned i = 0; i != mesh->mNumVertices; i++) {
    ...
    vertices.push_back({.pos = vec3(v.x, v.z, v.y),
                        .n = vec3(n.x, n.y, n.z),
                        .tc = vec2(t.x, t.y)});
  }
  ...
  glNamedBufferStorage(modelMatrices, sizeof(mat4) * 2, nullptr,
                       GL_DYNAMIC_STORAGE_BIT);
  ...
  GLuint cubemapTex;
  {
    int w, h, comp;
    const float* img =
        stbi_loadf("data/piazza_bologni_4k.hdr", &w, &h, &comp, 3);
    ...
    image::Bitmap in(w, h, comp, image::eBitmapFormat_Float, img);
    image::Bitmap out = convertEquirectangularMapToVerticalCross(in);
    stbi_image_free((void*)img);

    image::Bitmap cubemap = convertVerticalCrossToCubeMapFaces(out);
    ...
    for (unsigned i = 0; i != 6; ++i) {
      glTextureSubImage3D(cubemapTex, 0, 0, 0, i, cubemap.w_, cubemap.h_, 1,
                          GL_RGB, GL_FLOAT, data);
      data += cubemap.w_ * cubemap.h_ * cubemap.comp_ *
              image::Bitmap::getBytesPerComponent(cubemap.fmt_);
    }
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    PerFrameData perFrameData = {.view = view,
                                 .proj = p,
                                 .cameraPos =
                                     glm::vec4(camera3d.getPosition(), 1.0f)};
    glNamedBufferSubData(perFrameDataBuffer, 0, kUniformBufferSize,
                         &perFrameData);
    ...
    glDrawElementsInstancedBaseVertexBaseInstance(
        GL_TRIANGLES, static_cast<unsigned>(indices.size()), GL_UNSIGNED_INT,
        nullptr, 1, 0, 0);

    progCube.use();
    perFrameData.view = glm::mat4(glm::mat3(
        perFrameData.view)); // remove translation when drawing cube map
    glNamedBufferSubData(perFrameDataBuffer, 0, kUniformBufferSize,
                         &perFrameData);
