...
void uploadBufferData(const VulkanRenderDevice& vkDev,
                      const VkDeviceMemory& bufferMemory,
                      VkDeviceSize deviceOffset, const void* data,
                      const size_t dataSize);

bool createTextureImageFromData(const VulkanRenderDevice& vkDev,
...
bool loadTextureFromFile(const VulkanRenderDevice& vkDev, const char* filename,
                         VkImage& textureImage, VkFormat imageFormat,
                         VkDeviceMemory& textureImageMemory,
                         uint32_t* outTexWidth = nullptr,
                         uint32_t* outTexHeight = nullptr);

bool createVulkanImage(const VulkanRenderDevice& vkDev, const char* filename,
                       VulkanImage& image);
} // namespace vkFramework