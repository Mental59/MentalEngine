...
namespace
{
void setupLayers(std::vector<const char*>* layers, VkInstanceCreateInfo* instanceCreateInfo);
void addValidationLayers(std::vector<const char*>* layers);
void checkLayersSupport(const char** layers, uint32_t size);

void setupExtensions(std::vector<const char*>* extensions, VkInstanceCreateInfo* instanceCreateInfo);
...
#if defined(_DEBUG)
	VkDebugUtilsMessengerCreateInfoEXT createInfo = mental::debugUtilsMessengerCreateInfo();
...
void checkLayersSupport(const char** layers, uint32_t size)
{
	...
	for (uint32_t i = 0; i < size; i++)
	{
		...
		if (!layerFound)
		{
			throw std::runtime_error(std::format("Layer {} is not supported", layers[i]));
		}
	}
}
...
void addMainExtensions(std::vector<const char*>* extensions)
{
	extensions->insert(extensions->end(), {"VK_KHR_surface", VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME});
}

void addDebugExtensions(std::vector<const char*>* extensions)
{
	extensions->insert(extensions->end(), {VK_EXT_DEBUG_UTILS_EXTENSION_NAME, VK_EXT_DEBUG_REPORT_EXTENSION_NAME});
}
...
void checkExtensionsSupport(const char** extensions, uint32_t size)
{
	...
	for (uint32_t i = 0; i < size; i++)
	{
		...
		if (!extensionFound)
		{
			throw std::runtime_error(std::format("Extension {} is not supported", extensions[i]));
		}
	}
}
}  // namespace
